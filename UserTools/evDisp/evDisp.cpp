...
bool evDisp::Initialise(std::string configfile, DataModel &data){
	...
	if(plotStyle==0){
		...
		hitsVTimeHist = new TH1F("hitsVTimeHist", "# of hits vs time", 200, 250, 2000);
	...
	*/
	...
	gStyle->SetOptStat(0);   // disable stats box be a decent task to start of with just to remind me where everything is in the code
...
//	Double_t b[9]     = { 0.5293, 0.8684, 0.8385, 0.7914, 0.6425, 0.4662, 0.3499, 0.1968, 0.0539 };
	...
	// on the SK website.
	Double_t stops[17] = {0.0000, 0.0083, 0.0166, 0.0291, 0.0458, 0.0667, 0.0908, 0.1183, 0.1500, 0.1850, 0.2242, 0.2667, 0.3125, 0.3625, 0.3950, 0.4150, 1.0000};  //XXX
...
//			std::cout<<"drawing hits vs charge"<<std::endl;
	...
	// common block for reconstructed info - skroot_lowe - but is not populated
	if(dataSrc==1){
...
int evDisp::FindHits(){
	...
	if(myTreeReader->GetTree()->FindBranch("TQREAL")){
		...
		myTreeReader->Get("TQREAL", myTQReal);
			std::cout << "populated! Setting dataSrc = 1 to pull hits from TQReal." << std::endl;
		if(myTQReal->nhits > 0){
			// TQREAL has hits so set dataSrc to 1 (use TQReal) and return 1
			dataSrc = 1;
			return 1;
		} else std::cout << "not populated." << std::endl << "Looking for hits in sktqz_ which is ";
		// no TQReal hits so check the sktqz_ common block
	} else if(sktqz_.nqiskz > 0){
		std::cout << "populated! Setting dataSrc = 0 to pull hits from sktqz_." << std::endl;
		// sktqz_ has hits so set dataSrc to 0 (use sktqz_) and return 1
		dataSrc = 0;
		return 1;
	} else if(sktqz_.nqiskz == 0){
		std::cout << "not populated." << std::endl << "Looking for hits in skt_ and skq_ which are ";
		// no sktqz_ hits so check the skq_ and skt_ common blocks
	} else if(skt_.tisk && skq_.qisk){
		std::cout << "populated! Setting dataSrc = 2 to pull hits from skt_ and skq_." << std::endl;
		// skt_ and skq_ common blocks have information so set dataSrc 2 (use skt_ and skq_) and return 1
		dataSrc = 2;
		return 1;
	}
...
bool evDisp::BranchCheck(){
	...
	if(myTreeReader->GetTree()->FindBranch("SECONDARY")){
		...
		} else std::cout << "unpopulated." << std::endl;
	} else std::cout << "The secondaries branch is not present." << std::endl;
	...
	if(myTreeReader->GetTree()->FindBranch("LOWE")){
		...
		// might be a better way to do this check but I can't see why using bsenergy wouldn't work?'
