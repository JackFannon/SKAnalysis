...
bool evDisp::Initialise(std::string configfile, DataModel &data) {
  ...
  if (dataSrc == -1 || dataSrc == 1) {
    ...
    if (m_data->Trees.count(treeReaderName) == 0) {
      Log("Failed to find TreeReader " + treeReaderName + " in DataModel!",
          v_error, verbosity);
      return false;
    } else {
  ...
  if (plotStyle == 0) {
    ...
    // Setting up the histograms for time and charge distributions, ranges might
    // want looking at in more detail.
    hitsVTimeHist =
        new TH1F("hitsVTimeHist", "# of hits vs time", 200, 250, 2000);
    hitsVTimeHist->SetFillColor(38);
    hitsVTimeHist->SetLineColor(38);
    hitsVTimeHist->GetXaxis()->SetTitle("Time (ns)");

    hitsVChargeHist =
        new TH1F("hitsVChargeHist", "Charge deposited per PMT", 100, 0, 10);
    ...
    topCapHeatMap =
        new TH2D("topCapHeatMap", "topCap", 50, -2000, 2000, 50, -2000, 2000);
    bottomCapHeatMap = new TH2D("bottomCapHeatMap", "bottomCap", 50, -2000,
                                2000, 50, -2000, 2000);
    barrelHeatMap = new TH2D("barrelSideHeatMap", "barrelSide", 150, -3.14,
                             3.14, 50, -2000, 2000);

    // Setting up the histograms for time and charge distributions, ranges might
    // want looking at in more detail.
    hitsVTimeHist =
        new TH1F("hitsVTimeHist", "# of hits vs time", 200, 950, 1400);
    hitsVTimeHist->SetFillColor(38);
    hitsVTimeHist->SetLineColor(38);
    hitsVTimeHist->GetXaxis()->SetTitle("Time (ns)");
  ...
  // canvas for plotting
  displayCanvas = new TCanvas(); // if we name it, name must be unique -
                                 // prevents duplicate Tools.
  ((TRootCanvas *)displayCanvas->GetCanvasImp())
      ->Resize(1024, 700); // FIXME make unique names automatically
  ...
  */
  ...
  gStyle->SetOptStat(0); // disable stats box be a decent task to start of with
                         // just to remind me where everything is in the code
  gStyle->SetPalette(
  ...
  // as SuperScan plots that can be found on the SK website.
  Double_t stops[17] = {0.0000, 0.0083, 0.0166, 0.0291, 0.0458, 0.0667,
                        0.0908, 0.1183, 0.1500, 0.1850, 0.2242, 0.2667,
                        0.3125, 0.3625, 0.3950, 0.4150, 1.0000}; // XXX
  Double_t r[17] = {0.4745, 0.3294, 0.1137, 0.3451, 0.3686, 0.3922,
                    0.5725, 0.6471, 0.8078, 0.9608, 0.9255, 0.8745,
                    0.8627, 0.8235, 0.8588, 0.9686, 1.0000};
  Double_t g[17] = {0.2078, 0.1686, 0.1255, 0.4196, 0.5373, 0.5490,
                    0.6980, 0.7924, 0.8863, 0.9882, 0.9412, 0.7333,
                    0.7255, 0.5804, 0.4275, 0.1255, 1.0000};
  Double_t b[17] = {0.8118, 0.8196, 0.8039, 0.8431, 0.4588, 0.1961,
                    0.2039, 0.2039, 0.2627, 0.3098, 0.2824, 0.2392,
                    0.2157, 0.1804, 0.1569, 0.1294, 1.0000};
  ...
  // XXX ROOT 5 doesn't seem to stretch the colour range properly to span the
  // full z axis range. So while kBird ought to run from dark blue to yellow,
  // the "default" (copied from ROOT 6) colour palette will be almost entirely
  // dark blue. We can mitigate this somewhat by plotting on a logarithmic
  // scale, and we can also do it by distorting the 'stops' as shown above. Best
  // results also seem to depend on the range of z values in the event... Based
  // on superscan, we clip the max charge range at 26.7, and combined with the
  // above stops we seem to get reasonable displays, if a little lacking in
  // colour.
  ...
  // if dataSrc is set to a negative value then call FindHits to look for a data
  // source that is populated.
  if (dataSrc == -1) {
    get_ok = FindHits();
    if (!get_ok) {
      Log("Couldn't find hits anywhere!", v_error, verbosity);
      return false;
    }
  }
...
bool evDisp::Execute() {
  ...
  /* rfm data files don't have TQREAL populated, even for events with a valid
  HEADER. Instead they have a valid TQLIST branch, which is a TClonesArray of TQ
  class objects. This gets read out by skrawread which calls TQRAWSK, which
  calls TQSKZ, which calls skroot_get_idtq to read the TQLIST branch. The
  returned data gets passed into to TQSKZ, which puts it into the common block
  array IQISKZ. After returning to TQRAWSK, this calls SKTQCONV_QB to do charge
  conversion from TDC/ADC counts to ns/p.e., and puts the result into TBUF_RAW
  (or QBUF_RAW, there are basically arrays of T, Q, and cable number here).
     Finally, skroot_set_tree calls skroot_set_tqreal to transfer the converted
  hits from QBUF_RAW etc. into the TQREAL branch. The upshot is TQLIST is used
  to populate TQREAL after conversion from TDC/ADC counts to ns/p.e. Once
  populated TQREAL has members `int nhits`, and a `std::vector<float> T, Q` for
  hit times and charges and a `std::vector<int> cables` for cable numbers of the
  hits. following from $SKOFL_ROOT/inc/sktq.h:

  *     ---------
  *     Contents of /SKHEADQB/:
  *     ---------
  *     IT0SK   ; Original T0 of the event
  *     IT0XSK  ; T0 of the event for
  ITISKZ,ITASKZ,ITABSK,TISK,IHCAB,TASK,IHACAB,...
  ...
  */
  ...
  /*	if(dataSrc==0){
                  // if we have both SHE and AFT available, load the appropriate
     common block data if(evtSrc==0) m_data->LoadSHE(treeReaderName); else
     m_data->LoadAFT(treeReaderName);
          }*/
  if (dataSrc == 1)
  ...
  default: {
    // unknown
    Log(toolName + " unknown dataSrc: " + std::to_string(dataSrc), v_error,
        verbosity);
  ...
  Log(toolName + " This event had " + std::to_string(totalPMTsActivated) +
          " PMTs hit",
      v_debug, verbosity);
  ...
  if (totalPMTsActivated == 0) {
    // HEADER branch
    // extract the run number, subrun number, number of events and the trigger
    // type from the HEADER branch
    ...
    for (int pmtNumber = 0; pmtNumber < totalPMTsActivated; ++pmtNumber) {
      ...
      case 0: {
        ...
        in_gate = sktqz_.ihtiflz[pmtNumber] &
                  0x02; // sktqaz_.ihtflz[pmtNumber]  for OD
      ...
      case 1: {
        ...
        // convert hit time within readout window to hit time within subtrigger
        // window by subtracting the time from 40us buffer readout start to
        // subtrigger start then convert TDC ticks to nanoseconds COUNT_PER_NSEC
        // is a #defined constant in '$SKOFL_ROOT/inc/skheadC.h' describing
        // conversion from TDC ticks to nanoseconds
        time = time - (it0xsk - it0sk) / COUNT_PER_NSEC;
        // HEADER branch
        // extract the run number, subrun number, number of events and the
        // trigger type from the HEADER branch
        runNum = myHeader->nrunsk;
        subrunNum = myHeader->nsubsk;
        eventNum = myHeader->nevsk;
        readoutT0 = myHeader->t0;
      ...
      case 2: {
        ...
        // note index for skq and skt has to be cableNumber - 1 to deal with the
        // conversion from fortran indexing to c indexing
      ...
      */
      ...
      // can't seem to see a #defined constant for OD tubeID starts... it's
      // 20001 from above
      if (cableNumber > MAXPM)
        continue;
      // see above IHTIFLZ definition for in_gate test
      if (inGateOnly && (std::bitset<8 * sizeof(int)>(in_gate).test(1) == 0))
        continue;

      // get tube position
      myConnectionTable->GetTubePosition(cableNumber, tubePosition);
      tubeRadialCoordinate =
          sqrt(pow(tubePosition[0], 2.f) + pow(tubePosition[1], 2.f));
      tubeAngularCoordinate = acos(tubePosition[0] / tubeRadialCoordinate);
      if (tubePosition[1] > 0)
        tubeAngularCoordinate = -tubeAngularCoordinate;
      if (verbosity > 10) {
        FLAG++;
        std::cout << "hit on PMT " << cableNumber << std::endl;
        std::cout << "charge is " << charge << ", time is " << time
                  << ", PMT position is " << tubePosition[0] << ", "
                  << tubePosition[1] << ", " << tubePosition[2] << std::endl;
      }
      ...
      if (tubePosition[2] == zMax) {
        //			std::cout<<"setting top cap point
        //{"<<tubePosition[0]
        //					 <<", "<<tubePosition[1]<<",
        //"<<var<<"}"<<std::endl;
        if (topCapHeatMap)
          topCapHeatMap->Fill(tubePosition[0], tubePosition[1], var);
        if (topCapHitMap)
          topCapHitMap->SetPoint(top_cap_pmts_hit, tubePosition[0],
                                 tubePosition[1], var);
        if (var < top_cap_range.first)
          top_cap_range.first = var;
        if (var > top_cap_range.second)
          top_cap_range.second = var;
        ++top_cap_pmts_hit;
      } else if (tubePosition[2] == zMin) {
        //			std::cout<<"setting bottom cap point
        //{"<<tubePosition[0]
        //					 <<", "<<tubePosition[1]<<",
        //"<<var<<"}"<<std::endl;
        if (bottomCapHeatMap)
          bottomCapHeatMap->Fill(tubePosition[0], tubePosition[1], var);
        if (bottomCapHitMap)
          bottomCapHitMap->SetPoint(bottom_cap_pmts_hit, tubePosition[0],
                                    tubePosition[1], var);
        if (var < bot_cap_range.first)
          bot_cap_range.first = var;
        if (var > bot_cap_range.second)
          bot_cap_range.second = var;
        ++bottom_cap_pmts_hit;
      } else {
        //			std::cout<<"setting barrel point
        //{"<<tubeAngularCoordinate
        //					 <<", "<<tubePosition[2]<<",
        //"<<var<<"}"<<std::endl;
        if (barrelHeatMap)
          barrelHeatMap->Fill(tubeAngularCoordinate, tubePosition[2], var);
        if (barrelHitMap)
          barrelHitMap->SetPoint(barrel_pmts_hit, tubeAngularCoordinate,
                                 tubePosition[2], var);
        if (var < barrel_range.first)
          barrel_range.first = var;
        if (var > barrel_range.second)
          barrel_range.second = var;
        ++barrel_pmts_hit;
      }

      // filling histograms of charge and time
      if (hitsVTimeHist)
        hitsVTimeHist->Fill(time);
      if (hitsVChargeHist)
        hitsVChargeHist->Fill(charge);
    }
  }
  ...
  if (plotStyle == 0) {
    ...
    // set a dummy point so we still get the histogram axes, rather than empty
    // space
    if (top_cap_pmts_hit > 3) {
      //			std::cout<<"setting N top cap hits to
      //"<<top_cap_pmts_hit<<std::endl;
    ...
    if (bottom_cap_pmts_hit > 3) {
      //			std::cout<<"setting N bottm cap hits to
      //"<<bottom_cap_pmts_hit<<std::endl;
    ...
    if (barrel_pmts_hit > 3) {
      //			std::cout<<"setting N barrel hits to
      //"<<barrel_pmts_hit<<std::endl;
  ...
  // Print the event number, run number, sub run number and gate width for each
  // event
  std::cout << std::endl
            << "Event " << eventNum << " was from run " << runNum << ", subrun "
            << subrunNum << "." << std::endl;
  ...
  if (dataSrc == 1) {
    ...
    time_t runtime =
        mktime(&rundate); // need to use mktime to derive day of week
  ...
  std::cout << "Gate width/recorded readout window: " << gateWidth << " ns"
            << std::endl;

  // print out the value for T0SK. Units need to be added.
  std::cout << "Readout IT0SK: " << readoutT0 / (COUNT_PER_NSEC * 1000000)
            << " ms" << std::endl;

  // check for dataSrc being the TQReal branch before printing reconstructed
  // info. There does seem to be a common block for reconstructed info -
  // skroot_lowe - but is not populated
  if (dataSrc == 1) {
    // if the LOWE branch has been found to be present and populated then
    // printout the 4-vector reconstructed vertex
    if (recEnergy) {
      // vertex and energy are reconstructed independently where vertex recon
      // can pass but energy recon can fail so have to check separately if
      // reconstruction does not go ahead a default value of 9999 passes for
      // each variable if one value in vertex is still set at 9999 all four are
      // generally 9999 so only check one value
      if (recVertex[0] == 9999) {
        std::cout << "The vertex for this event could not be reconstructed."
                  << std::endl;
      } else {
      ...
      // similarly as for the vertex printout check one element for
      // non-reconstruction
      if (recDirection[0] == 9999) {
        std::cout << "The direction for this event could not be reconstructed."
                  << std::endl;
      } else {
      ...
      // check energy for a failed/not tried reconstruction and if it isn't 9999
      // then print
      if (recEnergy != 9999) {
        std::cout << "Reconstructed energy: " << recEnergy << " MeV"
                  << std::endl;
      } else
        std::cout << "The energy for this event could not be reconstructed."
                  << std::endl;
    }
  }
  ...
  // reset the charge and time histograms, this is required or else the bins
  // never empty and the histograms are filled with information from every event
  // cummulatively
  if (hitsVChargeHist)
    hitsVChargeHist->Reset("ICESM");
  if (hitsVTimeHist)
    hitsVTimeHist->Reset("ICESM");
...
bool evDisp::Finalise() {
  ...
  if (topCapHitMap)
    delete topCapHitMap;
  if (bottomCapHitMap)
    delete bottomCapHitMap;
  if (barrelHitMap)
    delete barrelHitMap;
  // histograms
  if (topCapHeatMap)
    delete topCapHeatMap;
  if (bottomCapHeatMap)
    delete bottomCapHeatMap;
  if (barrelHeatMap)
    delete barrelHeatMap;
...
int evDisp::FindHits() {
  std::cout << std::endl << "Looking for data sources... ";
  // first check for TQReal
  if (myTreeReader->GetTree()->FindBranch("TQREAL")) {
    std::cout << "TQReal branch exists and is ";
    // TQREAL branch exists, now check if it is populated with hits
    myTreeReader->Get("TQREAL", myTQReal);
    std::cout << "populated! Setting dataSrc = 1 to pull hits from TQReal."
              << std::endl;
    if (myTQReal->nhits > 0) {
      // TQREAL has hits so set dataSrc to 1 (use TQReal) and return 1
      dataSrc = 1;
      return 1;
    } else
      std::cout << "not populated." << std::endl
                << "Looking for hits in sktqz_ which is ";
    // no TQReal hits so check the sktqz_ common block
  } else if (sktqz_.nqiskz > 0) {
    std::cout << "populated! Setting dataSrc = 0 to pull hits from sktqz_."
              << std::endl;
    // sktqz_ has hits so set dataSrc to 0 (use sktqz_) and return 1
    dataSrc = 0;
    return 1;
  } else if (sktqz_.nqiskz == 0) {
    std::cout << "not populated." << std::endl
              << "Looking for hits in skt_ and skq_ which are ";
    // no sktqz_ hits so check the skq_ and skt_ common blocks
  } else if (skt_.tisk && skq_.qisk) {
    std::cout
        << "populated! Setting dataSrc = 2 to pull hits from skt_ and skq_."
        << std::endl;
    // skt_ and skq_ common blocks have information so set dataSrc 2 (use skt_
    // and skq_) and return 1
    dataSrc = 2;
    return 1;
  }
...
bool evDisp::BranchCheck() {
  ...
  if (myTreeReader->GetTree()->FindBranch("SECONDARY")) {
    ...
    std::cout << "The secondary branch is present and is ";
    myTreeReader->Get("SECONDARY", mySecondary);
    if (mySecondary->nscndprt > 0) {
      // secondary branch is populated!
      std::cout << "populated!" << std::endl;
      // secondary branch is unpopulated
    } else
      std::cout << "unpopulated." << std::endl;
  } else
    std::cout << "The secondaries branch is not present." << std::endl;

  // check the LOWE branch exists
  if (myTreeReader->GetTree()->FindBranch("LOWE")) {
    // it exists! now check if it is populated. Do this by checking if bsenergy
    // has a value" might be a better way to do this check but I can't see why
    // using bsenergy wouldn't work?'
    std::cout << "The LowE branch is present and is ";
